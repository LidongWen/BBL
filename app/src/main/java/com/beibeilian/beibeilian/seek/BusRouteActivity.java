...
public class BusRouteActivity extends Activity implements AMap.OnMapClickListener, AMap.OnMarkerClickListener,
	...
	protected void onCreate(Bundle bundle) {
		...
		mStartPoint = new LatLonPoint(Double.parseDouble(slat), Double.parseDouble(slng));
		mEndPoint = new LatLonPoint(Double.parseDouble(elat), Double.parseDouble(elng));
	...
	public void searchRouteResult(int routeType, int mode) {
		...
		if (routeType == ROUTE_TYPE_BUS) {// 公交路径规划
			RouteSearch.BusRouteQuery query = new RouteSearch.BusRouteQuery(fromAndTo, mode, mCurrentCityName, 0);// 第一个参数表示路径规划的起点和终点，第二个参数表示公交查询模式，第三个参数表示公交查询城市区号，第四个参数表示是否计算夜班车，0表示不计算

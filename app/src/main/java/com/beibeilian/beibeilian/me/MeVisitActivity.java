...
public class MeVisitActivity extends Activity implements PullableListView.OnLoadListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		seekPersonListAdapter = new SeekPersonListTimeAdapter(
				MeVisitActivity.this, personlist);
		btnBack.setOnClickListener(new OnClickListener() {

			@Override
			public void onClick(View v) {
				// TODO Auto-generated method stub
				startActivity(new Intent(MeVisitActivity.this,
						MainActivity.class));
				finish();
			}
		});
		...
		if(HelperUtil.flagISNoNull(VISIT_ON)&&VISIT_ON.equals("1"))
		{
			checkMemberState();
		}
		else
...
//		fn fnbanner=new fn(MeVisitActivity.this,BBLConstant.PANDASDK_ID, false);
//		fnbanner.setAttachedActivity(MeVisitActivity.this);
//		fnbanner.setAttachedViewId(R.id.rl_visit_banner);
			...
			public void run() {
				...
				try {
					...
					if (jsonObject.optInt("result") == BBLConstant.MEMBER_STATE_NUMBER_OUT) {
						Intent intent = new Intent(MeVisitActivity.this, OrderDailog.class);
		...
		public void run() {
			try {
				...
				message.obj = HelperUtil.postRequest(
						HttpConstantUtil.FindVisiter, map);
				message.what = 1;
		...
		try {
			...
			if (jsonArray.length() > 0) {
				...
				for (int i = 0; i < jsonArray.length(); i++) {
					...
					int heartduibaistate = jsonArray.optJSONObject(i).optInt(
							"heartduibaistate");
